...
func main() {
  ...
  turnover_type_key := "turnover.turnoverType"

  file_count := 0
  enum := 0
  input_path := "/home/kendall/Development/mfs_data/NBA_play_by_play/2017"
  output_file := "/home/kendall/Development/sports_aggregator_design/data_generator/enumerate/output/turnoverType.json"
  ...
  for _, file := range input_files { // ITERATE OVER FILES
    ...
    for _, play := range plays.json_objs {
      turnover_type, _, _ := find(play, turnover_type_key)
      if _, ok := output_json.key_value[*turnover_type]; !ok {
        output_json.key_value[*turnover_type] = fmt.Sprintf("%d", enum)
        enum += 1
      }
    }
