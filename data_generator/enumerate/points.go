...
func main() {
  ...
  points_key := "fieldGoalAttempt.Points"

  file_count := 0
  enum := 0
  input_path := "/home/kendall/Development/mfs_data/NBA_play_by_play/2017"
  output_file := "/home/kendall/Development/sports_aggregator_design/data_generator/enumerate/output/points.json"
  ...
  for _, file := range input_files { // ITERATE OVER FILES
    ...
    for _, play := range plays.json_objs {
      points, _, _ := find(play, points_key)
      if _, ok := output_json.key_value[*points]; !ok {
        output_json.key_value[*points] = fmt.Sprintf("%d", enum)
        enum += 1
      }
    }
